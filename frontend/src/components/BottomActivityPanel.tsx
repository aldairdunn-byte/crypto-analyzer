  ...
  return (
    ...
    >
        ...
        <div className="flex space-x-1 overflow-x-auto no-scrollbar min-w-0">
          {[
            { id: 'BOTS', label: 'Bots', labelFull: 'Mis Bots', count: bots.length, icon: PhosphorRobot, color: 'text-amber-400' },
            {
              id: 'POSITIONS',
              label: 'Posiciones',
              labelFull: 'Posiciones y Órdenes',
              count: openTrades.length + pendingTrades.length,
              icon: ChartLineUp,
      ...
      {!isCollapsed && (
        ...
        {/* ─── TAB 2: POSICIONES ABIERTAS Y ÓRDENES PROGRAMADAS ─── */}
        {activeTab === 'POSITIONS' && (
                            ...
                            return (
                              <tr key={ord.id} className="hover:bg-white/[0.03] transition-colors h-9">
                                ...
                                <td className="text-slate-400 text-[10.5px] tabular-nums font-bold">
                                  {diffPct > 0 ? `-${diffPct.toFixed(2)}% retroceso` : `+${Math.abs(diffPct).toFixed(2)}% ruptura`}
                                </td>
                      ...
                      <table className="w-full text-left min-w-[700px] whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-white/5 font-mono">
                          {openTrades.map((pos) => {
                            ...
                            const pnlPct = pos.entry_price > 0 ? ((curP - pos.entry_price) / pos.entry_price) * 100 : 0;
                            ...
                            return (
                              <tr key={pos.id} className="hover:bg-white/[0.03] transition-colors h-9">
                                ...
                                {/* ── Asesor de Salida (¿Cuándo Vender?) ── */}
                                <td>
                                  <div
                                ...
                                {/* ── Acciones: Vender 50% + Cerrar Mercado ── */}
                                <td className="text-right pr-4">
        ...
        {activeTab === 'TRADES' && (() => {
          const totalFeesPaid = trades.reduce((sum, tr) => {
            const fee = typeof tr.fee_usd === 'number'
              ? tr.fee_usd
          ...
          <div className="space-y-2">
            {/* Trades Subheader */}
            <div className="flex items-center justify-between px-1 pb-1 border-b border-white/[0.06] flex-wrap gap-2">
              <div className="flex items-center space-x-2 text-[11px] font-mono text-slate-400 flex-wrap gap-y-1">
                ...
                <span className="bg-amber-500/10 text-amber-400 border border-amber-500/25 px-2.5 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 shadow-xs">
                  <span>🏷️ Fees Pagados:</span>
                  <span className="font-black">-{formatDynamicPrice(totalFeesPaid, 2, currencyMode, penRate)}</span>
                  <span className="text-[9px] text-amber-400/70 font-normal hidden sm:inline">(Tasa VIP0 0.10% / 0.20% RT)</span>
                </span>
              </div>
